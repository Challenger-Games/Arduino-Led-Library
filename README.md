# Arduino-Led-Library
Its free and simple to use.

# How to install
Drop the folder in the libraries folder in the Arduino folder.

# How to use
First, import the library. Code: #include <LedLib.h>
Second, create an object of it. Code: LedLib lb(pin number here of led);
Third, play with with the functions. Functions: BLINKS(count), PULSE(delayms), FADEON(delayms), FADEOFF(delayms ms means mili secconds),

# Notes
It's just a simple library right now so don't try to push it to hard!

# Have fun with the engine
